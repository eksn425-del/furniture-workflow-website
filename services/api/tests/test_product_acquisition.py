...
# --- L2 access-challenge classification -----------------------------------
# Regression for the 45-site run: a CDN/WAF bot block states both
# "Request blocked" and the generic "Try again later" boilerplate.  It must be
# classified as an access denial (operator must change access), never as a
# temporary failure (operator told to retry something that can never succeed).


class _FakeLocator:
    def __init__(self, count: int = 0, body: str = "") -> None:
        self._count = count
        self._body = body
...
class _FakePage:
    """Minimal Playwright page double exposing only what the classifier reads."""

    def __init__(self, *, title: str, body: str, challenge_controls: int = 0) -> None:
        self._title = title
        self._body = body
        self._challenge_controls = challenge_controls
        self.reloads = 0
    ...
    def locator(self, selector: str) -> _FakeLocator:
        ...
        return _FakeLocator(self._challenge_controls)
...
CLOUDFRONT_BLOCK_TITLE = "ERROR: The request could not be satisfied"


def _collector(tmp_path: Path) -> "NativeBrowserCollector":
    from app.services.product_acquisition import NativeBrowserCollector

    return NativeBrowserCollector(tmp_path / "session")


def test_waf_block_with_retry_boilerplate_is_access_denied_not_temporary(tmp_path: Path) -> None:
    from app.services.product_acquisition import BrowserAccessDenied, BrowserTemporaryFailure

    collector = _collector(tmp_path)
    page = _FakePage(title=CLOUDFRONT_BLOCK_TITLE, body=CLOUDFRONT_BLOCK_BODY)

    with pytest.raises(BrowserAccessDenied) as error:
        collector._resolve_challenge(page, "https://example.test/p/1", headless=True)

    assert error.value.reason_code == "ACCESS_DENIED"
    # The bounded auto-reload path must not be entered for a definitive block.
    assert page.reloads == 0
    assert not isinstance(error.value, BrowserTemporaryFailure)


def test_genuine_maintenance_page_still_temporary(tmp_path: Path) -> None:
    from app.services.product_acquisition import BrowserTemporaryFailure

    collector = _collector(tmp_path)
    page = _FakePage(
        title="Service Unavailable",
        body="We are performing scheduled maintenance. This service is temporarily unavailable. Please try again later.",
    )

    with pytest.raises(BrowserTemporaryFailure) as error:
        collector._resolve_challenge(page, "https://example.test/p/1", headless=True)

    assert error.value.reason_code == "TEMPORARY_PAGE_FAILURE"
    # Real temporary faults still get the bounded auto-reload attempts.
    assert page.reloads > 0


def test_explicit_captcha_control_still_reaches_visible_handoff(tmp_path: Path) -> None:
    from app.services.product_acquisition import _VisibleHandoffRequired

    collector = _collector(tmp_path)
    page = _FakePage(
        title="Just a moment...",
        body="Verify you are human by completing the action below.",
        challenge_controls=1,
    )

    with pytest.raises(_VisibleHandoffRequired):
        collector._resolve_challenge(page, "https://example.test/p/1", headless=True)
