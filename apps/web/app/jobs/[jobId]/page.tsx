...
const STAGES = [
  ...
  { key: "DISCOVERY", label: "产品发现", detail: "范围内发现、身份与去重" },
  { key: "MEDIA", label: "产品筛选与复核", detail: "媒体绑定、视觉/类目/日期审核" },
  { key: "DIMENSION", label: "尺寸与命名", detail: "官方/预估尺寸、品牌+四段式命名" },
  { key: "READY_POOL", label: "生产闸门·入库", detail: "Catalog / Model Input Lock" },
  { key: "PROVIDER", label: "建模生成", detail: "Safety、提交、轮询与恢复" },
...
export default function JobDetailRoute({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  const [callLimitRaw, setCallLimitRaw] = useState("1");
  ...
  async function startRunNow() {
    ...
    if (job.provider !== "OFF" && job.provider_safety !== "PRODUCTION_READY") {
      setCallLimitRaw(String(job.approved_provider_call_limit ?? job.target_value ?? 1));
      setAskCeiling(true);
      return;
    }
  ...
  async function acceptShortageAndResume() {
    if (!job) return;
    setStarting(true);
    setError(null);
    setActionMsg(null);
    try {
      await updateControlTarget(job.job_id, {
        action: "ACCEPT_SHORTAGE",
        reason: "操作员确认接受当前完成数量并继续交付",
      });
      const resumed = await resumeProduction(job.job_id);
      setRun(resumed.run);
      setActionMsg(resumed.started ? "已接受缺额并恢复同一 Job。" : `恢复状态：${resumed.reason ?? "等待运行"}`);
      await load();
    } catch (reason) {
      setError(readableConsoleError(reason));
    } finally {
  ...
  const providerSafetyBlocked = Boolean(run && run.status === "BLOCKED" && run.stage === "PROVIDER_SAFETY");
  const canResumeScan = Boolean(siteScan && ["HUMAN_REQUIRED", "TEMPORARY_FAILURE", "ACCESS_CHANGE_REQUIRED", "FAILED", "BROWSER_RUNTIME_NOT_INSTALLED", "PARTIAL", "BRAIN_NOT_CONFIGURED"].includes(siteScan.status));
