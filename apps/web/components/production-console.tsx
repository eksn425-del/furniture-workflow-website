...
export function NewJobWizard() {
  ...
  async function probeAndScan() {
    ...
    try {
      ...
      const created = await createControlJob({ source_url: url, title: displayTitle, goal: displayGoal, is_brand_library: isBrand, brand_name: isBrand ? brandName : "", target_mode: targetMode, target_value: targetMode === "ALL" ? null : targetValue, scope, category_allocation: allocation, allocation_strategy: strategy, spillover, category_ids: [], provider: "OFF" });
      setJob(created.job);
  ...
  async function savePolicy() {
    ...
    try {
      ...
      if (!current) {
        ...
        if (!displayTitle) displayTitle = `${host} 网站采集`;
        if (!displayGoal) displayGoal = "按用户选择的类目扫描公开产品证据并准备交付";
        const created = await createControlJob({ source_url: url, title: displayTitle, goal: displayGoal, is_brand_library: isBrand, brand_name: isBrand ? brandName : "", target_mode: targetMode, target_value: targetMode === "ALL" ? null : targetValue, scope, category_allocation: allocation, allocation_strategy: strategy, spillover, category_ids: selectedCategories, provider: "OFF" });
        current = created.job;
