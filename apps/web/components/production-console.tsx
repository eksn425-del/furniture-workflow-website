  ...
  useEffect(() => {
    void getControlSites().then(async (response) => {
      ...
      if (fromUrl) {
        ...
        const matched = response.items.find((site) => !!host && norm(site.domain) === host && (site.categories?.length ?? 0) > 0);
...
export function SitesPage() {
  ...
  return <div className="console-page">
    ...
    {sites.length ? <div className="sites-grid">{sites.map((site) => {
      ...
      const countL1 = site.categories.filter((category) => catLevel(category) === 1).length;
      const countL2 = site.categories.filter((category) => catLevel(category) === 2).length;
      ...
      return <section className="console-panel site-card" key={site.site_key}>
        ...
        {scanPending ? <p className="site-card-scan-state"><i className="signal-dot signal-dot--attention" />类目扫描中（{scanStatus}），完成后自动刷新</p> : null}
