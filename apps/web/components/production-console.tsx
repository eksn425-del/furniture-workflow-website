...
/* eslint-disable react/no-unescaped-entities -- Chinese UI copy intentionally uses quoted labels. */
...
const STATUS_LABELS: Record<string, string> = {
  ...
  PROFILE_DRAFT: "站点画像草稿",
  VALIDATED: "站点画像已验证",
  PROFILE_STALE: "站点画像需重新学习",
  STALE: "站点画像已过期",
  PROFILE_BLOCKED: "站点画像受阻",
  UNSUPPORTED_STRATEGY_REQUIRED: "需要人工选择策略",
  OFFICIAL_FOUND: "官方尺寸已找到",
  OFFICIAL_ABSENT_CONFIRMED: "已确认官网无尺寸",
  OFFICIAL_LOOKUP_BLOCKED: "官方尺寸查询受阻",
  OFFICIAL_LOOKUP_TEMPORARY: "官方尺寸暂时不可用",
  OFFICIAL_LOOKUP_INCOMPLETE: "官方尺寸证据不完整",
  AGENT_READY: "Domain Agent 已完成",
  AGENT_FINISH_SCHEMA_INVALID: "Agent 输出需复核",
};
...
function tone(status: string) {
  ...
  if (["QUEUED", "SITE_SCAN_QUEUED", "HUMAN_REQUIRED", "WAITING_REVIEW", "TARGET_SHORTAGE", "UNVERIFIED", "STALE", "PROFILE_STALE", "OFFICIAL_LOOKUP_BLOCKED", "OFFICIAL_LOOKUP_TEMPORARY", "OFFICIAL_LOOKUP_INCOMPLETE"].includes(status)) return "attention";
...
// 与后端 _target_and_quotas 数学一致的单点镜像：实时计算每个已选类目将分到多少，
// 以及是否存在会阻断保存/启动的数量问题。任何 target/类目/策略变化都实时重算。
function planQuotaPreview(params: {
  targetMode: "EXACT_N" | "UP_TO_N" | "ALL";
  targetValue: number;
  allocation: "TOTAL_ACROSS_SELECTED" | "PER_CATEGORY";
  strategy: "SEQUENTIAL" | "EVEN" | "PROPORTIONAL" | "CUSTOM";
  categoryQuotas: Record<string, number>;
  selectedIds: string[];
  categories: ControlCategory[];
}): { perCategory: Record<string, number>; total: number; issues: string[] } {
  const { targetMode, targetValue, allocation, strategy, categoryQuotas, selectedIds, categories } = params;
  const issues: string[] = [];
  const perCategory: Record<string, number> = {};
  if (targetMode === "ALL") return { perCategory, total: 0, issues };
  if (selectedIds.length === 0) return { perCategory, total: 0, issues: ["尚未选择任何类目"] };
  const base = Math.max(1, Number(targetValue) || 1);
  if (allocation === "PER_CATEGORY") {
    selectedIds.forEach((id) => { perCategory[id] = base; });
    return { perCategory, total: base * selectedIds.length, issues };
  }
  // TOTAL_ACROSS_SELECTED：总计保持为目标 N。
  if (strategy === "EVEN") {
    const share = Math.floor(base / selectedIds.length);
    const remainder = base % selectedIds.length;
    selectedIds.forEach((id, index) => { perCategory[id] = share + (index < remainder ? 1 : 0); });
    return { perCategory, total: base, issues };
  }
  if (strategy === "PROPORTIONAL") {
    const weights: Record<string, number> = {};
    for (const id of selectedIds) {
      const category = categories.find((item) => item.category_id === id);
      const value = category?.count_value;
      if (!category || category.count_kind === "UNKNOWN" || typeof value !== "number" || value < 0) {
        issues.push(`类目「${category?.canonical_name ?? id}」数量未知，无法按比例分配`);
        weights[id] = 0;
      } else {
        weights[id] = value;
      }
    ...
    if (issues.length === 0 && weightSum > 0) {
      const raw: Record<string, number> = {};
      Object.entries(weights).forEach(([id, w]) => { raw[id] = (base * w) / weightSum; });
      Object.entries(raw).forEach(([id, r]) => { perCategory[id] = Math.floor(r); });
      let remainder = base - Object.values(perCategory).reduce((a, b) => a + b, 0);
      Object.keys(raw)
        .sort((a, b) => (raw[b] - Math.floor(raw[b])) - (raw[a] - Math.floor(raw[a])))
        .forEach((id) => { if (remainder > 0) { perCategory[id] += 1; remainder -= 1; } });
  ...
  if (strategy === "CUSTOM") {
    ...
    for (const id of selectedIds) {
      const value = Number(categoryQuotas[id] ?? 0);
      if (!Number.isInteger(value) || value < 0) issues.push("自定义配额必须是非负整数");
      quotas[id] = Math.max(0, value);
    }
    if (Object.keys(categoryQuotas).some((id) => !selectedIds.includes(id))) {
      issues.push("存在已填写配额但未选中的类目");
    }
    const sum = Object.values(quotas).reduce((a, b) => a + b, 0);
    if (sum !== base) issues.push(`自定义配额合计 ${sum} ≠ 目标 ${base}`);
    Object.assign(perCategory, quotas);
    return { perCategory, total: base, issues };
  }
  // SEQUENTIAL：按顺序补足，不预设每类目配额。
  selectedIds.forEach((id) => { perCategory[id] = 0; });
  return { perCategory, total: base, issues };
}
...
export function NewJobWizard() {
  ...
  const [dimensionPolicy, setDimensionPolicy] = useState<"FULL_ONLY" | "ALLOW_PARTIAL_ANCHOR">("FULL_ONLY");
  const [categoryQuotas, setCategoryQuotas] = useState<Record<string, number>>({});
  ...
  const ceilingRaw = "100";
  const [callLimitRaw, setCallLimitRaw] = useState("3");
  ...
  useEffect(() => {
    void getControlSites().then(async (response) => {
      ...
      if (fromUrl) {
        ...
        if (matchedSite?.is_brand_library) {
          setIsBrand(true);
          setBrandName(matchedSite.brand_name ?? "");
        }
  ...
  async function probeAndScan() {
    ...
    try {
      ...
      const created = await createControlJob({ source_url: url, title: displayTitle, goal: displayGoal, is_brand_library: isBrand, brand_name: isBrand ? brandName : "", target_mode: targetMode, target_value: targetMode === "ALL" ? null : targetValue, scope, category_allocation: allocation, allocation_strategy: strategy, spillover, category_quotas: strategy === "CUSTOM" ? categoryQuotas : {}, category_ids: [], provider: "OFF" });
      setJob(created.job);
  ...
  async function pollScan(jobId: string, attempt: number) {
    if (attempt === 120) setMessage("扫描仍在后台进行，继续等待结果；请勿重复创建。也可离开页面，在网站库查看同一扫描。");
    try {
    ...
    scanTimer.current = window.setTimeout(() => { void pollScan(jobId, attempt + 1); }, attempt >= 120 ? 10000 : 3000);
  }
  ...
  async function savePolicy() {
    ...
    try {
      ...
      if (!current) {
        ...
        if (!displayTitle) displayTitle = `${host} 网站采集`;
        if (!displayGoal) displayGoal = "按用户选择的类目扫描公开产品证据并准备交付";
        const created = await createControlJob({ source_url: url, title: displayTitle, goal: displayGoal, is_brand_library: isBrand, brand_name: isBrand ? brandName : "", target_mode: targetMode, target_value: targetMode === "ALL" ? null : targetValue, scope, category_allocation: allocation, allocation_strategy: strategy, spillover, category_quotas: strategy === "CUSTOM" ? categoryQuotas : {}, category_ids: selectedCategories, provider: "OFF" });
        current = created.job;
        setMessage(`任务已创建：${current.title}`);
      } else {
        let updated = current;
        // The Job is created before taxonomy scanning so the scan has a
        // durable owner.  Persist the operator's final strategy here as well;
        // otherwise a post-scan choice would remain UI-only.
        const policyUpdated = await updateControlJob(current.job_id, {
          category_allocation: allocation,
          allocation_strategy: strategy,
          spillover,
          category_quotas: strategy === "CUSTOM" ? categoryQuotas : {},
          dimension_anchor_policy: dimensionPolicy,
        });
        updated = policyUpdated.job;
        if (targetMode !== "ALL") {
          const res = await updateControlTarget(updated.job_id, { action: "MODIFY_TARGET", target_value: targetValue, reason: "操作员在目标与策略步骤确认目标数量" });
          updated = res.job;
        }
      ...
      if (current.policy.dimension_anchor_policy !== dimensionPolicy) {
        current = (await updateControlJob(current.job_id, { dimension_anchor_policy: dimensionPolicy })).job;
      }
  ...
  async function startProductionFlow() {
    if (!job) return;
    setBlocker(null);
    // Provider OFF 时直接启动；付费 Provider 必须由操作员显式确认成本上限。
    if (provider !== "OFF" && job.provider_safety !== "PRODUCTION_READY") {
      if (askCeiling) {
        await confirmPaywallCeiling();
        return;
      }
      setAskCeiling(true);
      const exactDefault = targetMode === "ALL"
        ? 1
        : targetValue * (allocation === "PER_CATEGORY" ? Math.max(1, selectedCategories.length) : 1);
      setCallLimitRaw(String(job.approved_provider_call_limit ?? exactDefault));
      setBlocker(`付费 Provider 将使用 ¥${ceilingRaw} 成本上限；未填写时按本次 Exact-N 目标（${exactDefault} 次）执行。请确认。`);
      return;
    }
  ...
  async function confirmPaywallCeiling() {
    ...
    try {
      ...
      const callLimit = Math.trunc(Number(callLimitRaw));
      if (!Number.isFinite(ceilingMinor) || ceilingMinor <= 0) throw new Error("成本上限必须是大于 0 的金额");
      if (!Number.isFinite(callLimit) || callLimit <= 0) throw new Error("Provider 硬调用上限必须是大于 0 的整数");
      const approval = await approveControlJob(job.job_id, { confirm: true, approved_cost_ceiling_minor: ceilingMinor, approved_provider_call_limit: callLimit, actor: "website-operator" });
      setJob(approval.job);
  ...
  // 子级收窄父级：一级被勾选且其下也有勾选的二级时，一级整棵不计，
  // 只计已勾选的二级数量；仅当一级无任何勾选二级时才计整棵合计。
  const selectedChildParentPaths = new Set(
    categories.filter((category) => selectedCategories.includes(category.category_id) && levelOfCategory(category) === 2)
      .map((category) => parentOfCategory(category)),
  );
  const selectedCount = categories
    .filter((category) => selectedCategories.includes(category.category_id))
    .reduce((sum, category) => {
      if (levelOfCategory(category) === 2) return sum + (category.count_value ?? 0);
      if (selectedChildParentPaths.has(category.path)) return sum;
      return sum + rootTotalOf(category);
    }, 0);
  const selectedHasUnknownCount = categories.some((category) => {
    if (!selectedCategories.includes(category.category_id) || category.count_kind !== "UNKNOWN") return false;
    return levelOfCategory(category) === 2 || !selectedChildParentPaths.has(category.path);
  });
  ...
  function toggleCategory(categoryId: string) {
    setSelectedCategories((current) => {
      ...
      if (!current.includes(categoryId)) setCategoryQuotas((quotas) => ({ ...quotas, [categoryId]: quotas[categoryId] ?? 1 }));
      return next;
    });
  }
  const customQuotaTotal = selectedCategories.reduce((sum, categoryId) => sum + Math.max(0, Number(categoryQuotas[categoryId] ?? 0)), 0);
  // 实时配额预览（与后端 _target_and_quotas 镜像），任何 target/类目/策略变化都重算。
  const quotaPreview = planQuotaPreview({
    targetMode, targetValue, allocation, strategy, categoryQuotas,
    selectedIds: selectedCategories, categories,
  });
  const quotaIssues = quotaPreview.issues;
  const quotaReady = selectedCategories.length > 0 && quotaIssues.length === 0;
  // 从网站库进入时（?from=sites），向导的返回应回到网站库，而不是回到「扫描新网站」第一步。
  const fromLibrary = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("from") === "sites";
...
export function SystemStatusPage() {
  ...
  const diagnosticValue = (value: string | undefined, fallback = busy ? "读取中…" : "未知") => value === "CODEX_BRAIN_NOT_CONNECTED" ? "本地大脑未连接" : value === "CODEX_BRIDGE_RECENT_RESPONSE" ? "近期有真实响应" : value ?? fallback;
  ...
  return <div className="console-page">
    <PageHeader eyebrow="系统 / 运行边界" title="系统健康与安全" description="这里看 Website 原生运行时、实际生效的大脑、L2 浏览器、Blender、Provider 安全和 Worker 状态。密钥、私有 URL、会话目录不会显示。" action={<Link className="console-button console-button--quiet" href="/">返回首页</Link>} />
    {error ? <div className="console-alert console-alert--danger"><strong>系统状态读取失败</strong><span>{error}</span></div> : null}
    {brainOverride?.status === "ON" || status?.website_brain.local_agent_override ? <div className="console-alert console-alert--attention"><strong>本地 Agent 复核已生效</strong><span>{brainOverride?.reason ?? status?.website_brain.override_reason ?? "当前复核由本地 Agent 完成，不发起外部大脑请求。"}</span></div> : null}
    <div className="system-health-grid">
      <HealthStatusCard title="网站 API" value={diagnosticValue(diagnostics?.api?.status, error ? "ERROR" : "READY")} detail="控制平面响应正常" tone={error ? "danger" : "success"} />
      <HealthStatusCard title="数据库" value={diagnosticValue(diagnostics?.database?.status ?? status?.database.status)} detail="状态来自实际 SELECT 1 检查" tone={diagnosticTone(diagnostics?.database?.status ?? status?.database.status)} />
      <HealthStatusCard title="L2 浏览器" value={diagnosticValue(l2?.status)} detail={`${l2?.engine ?? "chromium"} · ${l2?.mode ?? "ISOLATED_PERSISTENT"}`} tone={diagnosticTone(l2?.status)} />
      <HealthStatusCard title="网站大脑" value={diagnosticValue(brain?.status ?? (status?.website_brain.review_provider === "LOCAL_AGENT" ? "LOCAL_AGENT" : status?.website_brain.status))} detail={brain?.effective_mode === "LOCAL_AGENT" || status?.website_brain.review_provider === "LOCAL_AGENT" ? "本地 Agent · 不发外部 API" : status?.website_brain.model_mode === "CODEX_DEVELOPMENT_BRIDGE" ? "需要活动 Codex 会话响应；配置目录不等于自动大脑" : status?.website_brain.configured ? "已配置独立大脑" : "未配置 WEBSITE_BRAIN_*"} tone={diagnosticTone(brain?.status ?? status?.website_brain.status)} />
    </div>
    <div className="system-grid">
      <section className="console-panel">
        <SectionTitle title="实际运行边界" detail="所有状态都来自当前进程、运行时探测或持久化数据库；不因配置模板而伪造 READY。" />
        <div className="system-kv">
          <div><span>大脑生效模式</span><strong>{diagnosticValue(brain?.effective_mode ?? status?.website_brain.model_mode)}</strong></div>
          <div><span>模式来源</span><strong>{diagnosticValue(brain?.effective_mode_source ?? status?.website_brain.mode_source)}</strong></div>
          <div><span>视觉输入</span><strong>{diagnosticValue(brain?.vision_input ?? status?.website_brain.vision_input)}</strong></div>
          <div><span>大脑覆盖</span><strong>{diagnosticValue(brainOverride?.status, "OFF")}</strong></div>
          ...
          <div><span>Provider 安全</span><strong>{diagnosticValue(status?.provider.status)}</strong></div>
          <div><span>对象存储</span><strong>{diagnosticValue(status?.object_storage.status)}</strong></div>
          <div><span>大脑命名空间</span><strong>{status?.website_brain.namespace ?? "WEBSITE_BRAIN_*"}</strong></div>
          <div><span>大脑调用次数</span><strong>{String(status?.website_brain.provider_posts ?? 0)}</strong></div>
        </div>
      ...
      <section className="console-panel">
        ...
        <div className="health-rows">
          {Object.entries(workerRows).map(([name, value]) => <HealthRow key={name} label={`${name} 工作进程`} value={value} tone={diagnosticTone(value)} />)}
          <HealthRow label="生产工作进程" value={productionWorker?.status ?? "读取中…"} tone={diagnosticTone(productionWorker?.status)} />
        </div>
      </section>
    </div>
  </div>;
...
export function SitesPage() {
  ...
  return <div className="console-page">
    ...
    {sites.length ? <div className="sites-grid">{sites.map((site) => {
      ...
      const launchHref = `/jobs/new?url=${encodeURIComponent(site.source_url || `https://${site.domain}`)}&from=sites`;
...
export function SiteDetailPage({ siteKey }: { siteKey: string }) {
  ...
  return <div className="console-page">
    ...
    <PageHeader eyebrow="网站档案 / Site Detail" title={String(site.display_name ?? siteKey)} description="查看入口 URL、类目快照、数量证据、扫描历史和关联任务。网站扫描与生产 Job 的生命周期彼此独立。" action={<Link className="console-button console-button--primary" href={`/jobs/new?url=${encodeURIComponent(String(detail?.entry_urls?.[0]?.url ?? `https://${siteKey}`))}&from=sites`}>按此网站建任务 →</Link>} />
...
export function DeliveryPage() {
  ...
  return <div className="console-page">
    ...
    {deliveries.length ? <div className="delivery-grid">{deliveries.map((delivery) => <section className="console-panel delivery-card" key={delivery.delivery_id}>
      <div className="delivery-card-head"><div><h3>{delivery.title ?? delivery.delivery_id}</h3><p>{delivery.relative_path}</p></div><StatusBadge status="COMPLETED" /></div>
      <div className="site-card-stats"><div><span>批次</span><strong>{delivery.batch_count}</strong></div><div><span>模型总数</span><strong>{delivery.model_count}</strong></div><div><span>完成时间</span><strong>{formatTime(delivery.modified_at)}</strong></div></div>
