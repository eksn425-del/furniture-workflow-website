...
export function SystemStatusPage() {
  ...
  return <div className="console-page">
    ...
    <div className="system-health-grid">
      ...
      <HealthStatusCard title="网站大脑" value={diagnosticValue(brain?.status ?? (status?.website_brain.review_provider === "LOCAL_AGENT" ? "LOCAL_AGENT" : status?.website_brain.status))} detail={brain?.effective_mode === "LOCAL_AGENT" || status?.website_brain.review_provider === "LOCAL_AGENT" ? "本地 Agent · 不发外部 API" : status?.website_brain.configured ? "已配置独立大脑" : "未配置 WEBSITE_BRAIN_*"} tone={diagnosticTone(brain?.status ?? status?.website_brain.status)} />
    </div>
