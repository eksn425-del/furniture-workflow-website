...
export function NewJobWizard() {
  ...
  async function startProductionFlow() {
    ...
    if (provider !== "OFF" && job.provider_safety !== "PRODUCTION_READY") {
      ...
      setAskCeiling(true);
      setCallLimitRaw(String(job.approved_provider_call_limit ?? (targetMode === "ALL" ? 1 : targetValue)));
      setBlocker(`付费 Provider 将使用 ¥${ceilingRaw} 成本上限，并受 Provider 硬调用次数上限约束。请再次确认。`);
      return;
    }
  ...
  const customQuotaReady = strategy !== "CUSTOM" || (targetMode !== "ALL" && allocation === "TOTAL_ACROSS_SELECTED" && customQuotaTotal === targetValue && selectedCategories.every((categoryId) => Number.isInteger(Number(categoryQuotas[categoryId])) && Number(categoryQuotas[categoryId]) >= 0));
    ...
    <div className="system-grid">
      <section className="console-panel">
        ...
        <div className="system-kv">
          ...
          <div><span>Effective Mode Source</span><strong>{diagnosticValue(brain?.effective_mode_source ?? status?.website_brain.mode_source)}</strong></div>
          <div><span>Vision Input</span><strong>{diagnosticValue(brain?.vision_input ?? status?.website_brain.vision_input)}</strong></div>
