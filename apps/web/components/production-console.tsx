...
export function NewJobWizard() {
  ...
  const [callLimitRaw, setCallLimitRaw] = useState("5000");
  ...
  async function startProductionFlow() {
    ...
    if (provider !== "OFF" && job.provider_safety !== "PRODUCTION_READY") {
      ...
      setAskCeiling(true);
      setCallLimitRaw(String(job.approved_provider_call_limit ?? 5000));
      setBlocker(`付费 Provider 将使用 ¥${ceilingRaw} 成本上限；调用次数默认不设硬上限（5000，可按需修改）。请确认。`);
      return;
    }
