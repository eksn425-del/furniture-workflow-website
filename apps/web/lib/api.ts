...
// Prefer the same-origin Next proxy for local/embedded browsers. A direct
// loopback API URL is still supported when a deployment explicitly provides
// NEXT_PUBLIC_API_BASE_URL, but it is not a safe default for a browser tab
// whose page and API run in different network contexts.
